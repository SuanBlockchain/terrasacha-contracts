...
def validate_datum_update(old_datum: DatumProject, new_datum: DatumProject) -> None:
    ...
    assert old_datum.params.project_id == new_datum.params.project_id, "Project ID cannot be changed"
    
    # 3. Ensure that the protocol policy ID remains the same
    assert old_datum.protocol_policy_id == new_datum.protocol_policy_id, "Protocol policy ID cannot be changed"
    
    # 4. Ensure that the token name remains the same
    assert old_datum.project_token.token_name == new_datum.project_token.token_name, "Token name cannot be changed"
    
    # 5. Ensure that the project token policy ID remains the same
    assert old_datum.project_token.policy_id == new_datum.project_token.policy_id, "Project token policy ID cannot be changed"
    
    # 6. StakeHolders participation list remains the same
    assert len(old_datum.stakeholders) == len(new_datum.stakeholders), "Stakeholders list length cannot change"
    ...
    for i in range(len(old_datum.certifications)):
        ...
        assert old_cert.certification_date == new_cert.certification_date, "Existing certification date cannot change"
        assert old_cert.quantity == new_cert.quantity, "Existing certification quantity cannot change"
        # Real certification dates and quantities can only be added/updated, not removed
        assert new_cert.real_certification_date >= old_cert.real_certification_date, "Real certification date can only increase"
        assert new_cert.real_quantity >= old_cert.real_quantity, "Real certification quantity can only increase"
    ...
    assert total_participation == new_datum.project_token.total_supply, "Sum of participation must equal total supply"
    
    # 2. Current supply can only increase, and must always be <= total supply
    assert new_datum.project_token.current_supply >= old_datum.project_token.current_supply, "Current supply can only increase"
    ...
    assert new_datum.params.project_state >= old_datum.params.project_state, "Project state can only move forward"
...
def validator(protocol_policy_id: PolicyId, datum_project: DatumProject, redeemer: RedeemerProject, context: ScriptContext) -> None:
    ...
    assert datum_project.protocol_policy_id == protocol_policy_id, "Project datum protocol policy ID must match validator parameter"
