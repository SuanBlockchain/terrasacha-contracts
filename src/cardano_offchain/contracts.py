...
class ContractManager:
        ...
        try:
            ...
            for name, contract_data in saved_contracts.items():
                try:
                    ...
                    if (str(plutus_contract.testnet_addr) != contract_data['testnet_addr'] or 
                        str(plutus_contract.mainnet_addr) != contract_data['mainnet_addr'] or
                        plutus_contract.policy_id != contract_data['policy_id']):
                        continue
    ...
    def compile_contracts(self, address: pc.Address, force: bool = False) -> Dict[str, Any]:
        ...
        if (not force and self.contracts and self.compilation_utxo and 
            self._is_compilation_utxo_available(address)):
            return {
                'success': True,
        ...
        try:
            ...
            if not compiled_contracts:
                return {
                    'success': False,
                    'error': 'No contract files found to compile'
                }
    ...
    def get_contracts_info(self) -> Dict[str, Any]:
        ...
        contracts_info = {}
        for name, contract in self.contracts.items():
            contract_address = (contract.testnet_addr if self.chain_context.cardano_network == pc.Network.TESTNET 
