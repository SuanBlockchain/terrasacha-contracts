...
from terrasacha_contracts.minting_policies.authentication_nfts import Mint, Burn
...
class TokenOperations:
    ...
    def create_minting_transaction(
        ...
        try:
            ...
            builder.add_minting_script(
                script=minting_script, redeemer=pc.Redeemer(Mint())
            )
            ...
            signed_tx = builder.build_and_sign(
                [signing_key], change_address=from_address
            )
    ...
    def create_burn_transaction(
        ...
        try:
            ...
            builder.add_minting_script(
                script=minting_script, redeemer=pc.Redeemer(Burn())
            )
            ...
            signing_key = self.wallet.get_signing_key(0)
            signed_tx = builder.build_and_sign(
                [signing_key], change_address=user_address
            )
    ...
    def create_protocol_update_transaction(
        ...
        try:
            ...
            old_datum = DatumProtocol.from_cbor(
                protocol_utxo_to_spend.output.datum.cbor
            )
            ...
            if new_datum is None:
                ...
                new_datum = DatumProtocol(
                    ...
                    + 500000,  # Increase fee by 0.5 ADA
            ...
            protocol_output = pc.TransactionOutput(
                address=protocol_address,
                amount=pc.Value(
                    coin=min_val_protocol, multi_asset=protocol_multi_asset
                ),
                datum=new_datum,
            )
            builder.add_output(protocol_output)
