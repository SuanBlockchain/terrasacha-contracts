...
class MenuFormatter:
    ...
    def print_header(self, title: str, subtitle: str = None):
        ...
        print(Colors.HEADER + "║" + Colors.BOLD + f"{title:^{self.width-2}}" + Colors.ENDC + Colors.HEADER + "║" + Colors.ENDC)
        if subtitle:
            print(Colors.HEADER + "║" + Colors.OKBLUE + f"{subtitle:^{self.width-2}}" + Colors.ENDC + Colors.HEADER + "║" + Colors.ENDC)
    ...
    def print_menu_option(self, number: str, description: str, status: str = None):
        ...
        if status:
            status_color = (Colors.OKGREEN if status == "✓" else 
                           Colors.WARNING if status == "⚠" else 
                           Colors.FAIL if status == "✗" else Colors.ENDC)
            print(f"{Colors.OKBLUE}│{Colors.ENDC} {Colors.BOLD}{number:>2}{Colors.ENDC}. {description:<50} {status_color}[{status}]{Colors.ENDC}")
        else:
