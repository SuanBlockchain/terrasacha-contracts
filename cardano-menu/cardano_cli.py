...
from opshin.prelude import FalseData, TrueData
...
class WalletSelection:
    ...
    address: pc.Address
    wallet_name: str | None  # None if custom address was entered
...
class CardanoCLI:
    ...
    def resolve_address_input(self, input_str: str, switch_wallet: bool = False) -> str | None:
        """
    ...
    def display_wallet_info(self, show_all_wallets: bool = False):
        ...
        print("Wallet Type: HD Wallet (BIP32)")
        ...
        for wallet_name in wallets_to_show:
            ...
            print("DERIVED ADDRESSES (First 5):")
    ...
    def create_project_menu(self):
        ...
        while True:
            ...
            quantity_input = self.menu.get_input("Quantity of carbon credits for this certification period")
            try:
        ...
        for i, (name, participation, _stakeholder_pkh) in enumerate(stakeholders, 1):
    ...
    def update_protocol_menu(self):
        ...
        try:
            ...
            for utxo in protocol_utxos:
                if utxo.output.amount.multi_asset:
                    for policy_id, _assets in utxo.output.amount.multi_asset.data.items():
        ...
        _projects_name = current_datum.projects.copy()
    ...
    def mint_grey_tokens_menu(self):
        ...
        # Select minting mode
        self.menu.print_section("MINTING MODE SELECTION")
        self.menu.print_info("Choose minting mode:")
        print("│ 1. Free Minting (requires project NFT authorization token in wallet)")
        print("│ 2. Authorized Minting (any wallet, onchain contract validates authorization)")
        print()

        try:
            mode_choice = int(self.menu.get_input("Select minting mode (1-2)"))
            if mode_choice == 1:
                minting_mode = "free"
                self.menu.print_info("Selected: Free Minting (requires authorization token)")
            elif mode_choice == 2:
                minting_mode = "authorized"
                self.menu.print_info("Selected: Authorized Minting (contract validates)")
            else:
                self.menu.print_error("Invalid mode selection")
                input("Press Enter to continue...")
                return
        except (ValueError, IndexError):
            self.menu.print_error("Invalid input for mode selection")
            input("Press Enter to continue...")
            return
        ...
        if minting_mode == "free":
            ...
            try:
                min_purchase_input = self.menu.get_input("Enter minimum purchase amount (tokens, default: 1)")
    ...
    def burn_grey_tokens_menu(self):
        ...
        contracts = self.contract_manager.list_contracts()
        if not contracts:
            self.menu.print_error("No contracts available for grey token burning")
            input("Press Enter to continue...")
            return
        ...
        if not project_contracts:
            ...
            try:
                choice = (
                    int(
                        self.menu.get_input(
                            f"Select project contract for grey token burning (1-{len(project_contracts)})"
                        )
                    )
                    - 1
                )
        ...
        try:
            quantity_input = self.menu.get_input("Enter number of grey tokens to burn (default: 1)")
            burn_quantity = int(quantity_input) if quantity_input.strip() else 1
        ...
        current_wallet = self.wallet_manager.get_default_wallet_name()
        print(f"│ Project Contract: {selected_project}")
        print(f"│ Grey Tokens to Burn: {burn_quantity:,}")
        print(f"│ Wallet: {current_wallet} (containing tokens)")

        if not self.menu.confirm_action("Proceed with grey token burning?"):
            self.menu.print_info("Grey token burning cancelled")
            input("Press Enter to continue...")
            return
        ...
        try:
            ...
            if result["success"]:
                ...
                print(f"│ Quantity Burned: {result['burned_quantity']:,}")
    ...
    def cancel_grey_sale_menu(self):
        ...
        print("│ Action: Cancel sale and return grey tokens to seller")
    ...
    def update_grey_sale_price_menu(self):
        ...
        if not project_contracts:
            ...
            try:
                choice = (
                    int(self.menu.get_input(f"Select project contract for price update (1-{len(project_contracts)})"))
                    - 1
                )
                if 0 <= choice < len(project_contracts):
                    selected_project = project_contracts[choice]
        ...
        print("│ ")
        print("│ ⚠ WARNING: Transaction will fail if wallet doesn't match seller PKH in datum")
    ...
    def buy_grey_tokens_menu(self):
        ...
        try:
            ...
            print("│ ")
            print("│ ⚠ NOTE: Transaction requires USDATEST tokens in wallet")
            print("│ ⚠ Contract validates: amount >= min_purchase and amount <= available")
            print("│ ⚠ WARNING: Protocol fee is interpreted as USDA, not lovelace")
        ...
        if "stakeholders" in accumulated_changes:
            ...
            if added_names:
                ...
                for name in added_names:
                    ...
                    print(
                        f"│      • {sh_name} - PKH: {pkh_display} - Participation: {sh.participation:,} - Claimed: {claimed_display}"
                    )
                print("│")
    ...
    def accumulate_stakeholder_changes(self, current_datum, accumulated_changes):
        ...
        def stakeholders_are_equal(list1, list2):
            ...
            for s1, s2 in zip(list1, list2, strict=False):
                if (
    ...
    def display_transaction_changes(self, old_datum, new_datum):
        ...
        if len(old_datum.stakeholders) != len(new_datum.stakeholders):
            ...
            for i, (old_sh, new_sh) in enumerate(zip(old_datum.stakeholders, new_datum.stakeholders, strict=False)):
                if (
        ...
        if len(old_datum.certifications) != len(new_datum.certifications):
            ...
            for i, (old_cert, new_cert) in enumerate(
                zip(old_datum.certifications, new_datum.certifications, strict=False)
            ):
                if (
    ...
    def select_storage_type(self) -> str | None:
        """
    ...
    def select_reference_script_wallets(self, source_address: pc.Address) -> dict[str, Any] | None:
        """
    ...
    def compile_investor_contract_menu(self):
        ...
        for i, contract_name in enumerate(project_contracts, 1):
            ...
            if grey_contract:
                ...
                print("│    ✗ Grey Contract: Not compiled (compile with Option 4 first)")
    ...
    def edit_certification_in_list(self, certifications_list, project_state=0):
        ...
        try:
            ...
            self.menu.print_success("✓ Updated certification")
